...
$gender = array("--Select Gender--","Female","Male","Others");
  ...
  if(isset($_POST["submit"])){
    ...
    $user_country = $_POST["selected_country_option"];
    $user_gender = $_POST["selected_gender_option"];
    $security_key = $_POST["txtsecurity_key"];

    if($first_name!="" && $last_name!="" && $user_password!="" && $user_country!="" && $user_gender!="" && $security_key!=""){
      ...
      if($check_validity_status["status"] == "not_used"){
        $register_user_command = "INSERT INTO users_table (`Users_ID`, `user_fname`, `user_lname`, `gender`, `country`, `Password`, `Profile_Picture`) VALUES (NULL, '$first_name', '$last_name', '$user_gender', '$user_country', '$user_password', '')";
...
<body>
  ...
  <p style="font-size:.6em;color:#2dc6bf;text-align:center;cursor:hand"><u><a href="newlogin.php.php">Sign In</a></u></p>
